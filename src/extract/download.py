...
class Download:
	...
	# 下載後會以民國年月呈現
	def download(self,base_path,year,month,categories_idx,filename,foldername):
		...
		os.system(cmd)
	"""驗證下載檔案"""	
	def validation(self,base_path,year,month,categories_idx,filename,foldername):
		client_path = "%srawdata/%s/%d%02d.xls" % (base_path,foldername,year,month)
		#print client_path
		#os.system('clear')
	...
	def downloadAll():
		...
		#確保資料夾存在
		for fd in FOLDERS:
			self.checkFolder(base_path+"rawdata/"+fd+"/")
