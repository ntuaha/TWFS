...
class TRANSFORM_ATM:
	...
	columns = ["發行金融卡","流通金融卡","ATM裝設數","交易次數","交易金額"]	
	columns_en = ["Issue_Card_Cnt","F_Card_Cnt","ATM_Cnt","Txn_Cnt","Txn_Bal"]	
	...
	def parse(self):
		...
		for sheet_num in range(book.nsheets):
			...
			for i in range(sh.nrows):
				...
				self.total_data[2] = int(float(sh.cell_value(rowx=i,colx = 2)))
				self.total_data[3] = int(float(sh.cell_value(rowx=i,colx = 3)))
				self.total_data[4] = int(float(sh.cell_value(rowx=i,colx = 4)))
				self.total_data[5] = int(float(sh.cell_value(rowx=i,colx = 6))*1e6)
...
if __name__ == '__main__':
	#parser = TRANSFORM_ATM('/Users/aha/Dropbox/Project/Financial/Plan/rawdata/','/Users/aha/Dropbox/Project/Financial/Plan/data/')
	#parser.runParse('10001')
	#parser.runParse('10002')
	parser = TRANSFORM_ATM('/home/aha/Data/TWFS/rawdata/','/home/aha/Data/TWFS/data/')
	parser.parseAll()
