...
class Y_Bal:
	...
	def set(self,source_path,destination_path,date):
		self.source_path = source_path
		self.destination_path = destination_path
		self.date = date

	def parse(self,source_path,destination_path,date):
		self.set(source_path,destination_path,date)
		...
		for i in range(sh.nrows):
			...
			if row_name== u"總　　　　　計　Total" and 1 == mode:
				...
				rows.append([total_data[0],"總計",'全行外匯活期存款',self.column[0],total_data[1],self.columns_en[0]])
				rows.append([total_data[0],"總計",'全行外匯定期存款',self.column[1],total_data[2],self.columns_en[1]])
			...
			if 1 == mode:
				...
				rows.append([total_data[0],bank_name,"全行外匯活期存款",self.column[0],bank_data[bank_name]["ALL_MY"],self.columns_en[0]])
				rows.append([total_data[0],bank_name,"全行外匯定期存款",self.column[1],bank_data[bank_name]["ALL_FY"],self.columns_en[1]])
			...
			if sh.cell_value(rowx=i,colx = 0)== u"總　　　　　計　Total" and 2 == mode:
				...
				rows.append([total_data[0],"總計",'國內外匯活期存款',self.column[2],total_data[4],self.columns_en[2]])
				rows.append([total_data[0],"總計",'國內外匯定期存款',self.column[3],total_data[5],self.columns_en[3]])
				...
				rows.append([total_data[0],"總計",'海外外匯活期存款',self.column[4],total_data[7],self.columns_en[4]])
				rows.append([total_data[0],"總計",'海外外匯定期存款',self.column[5],total_data[8],self.columns_en[5]])
			...
			if 2 == mode:
				...
				rows.append([total_data[0],bank_name,'國內外匯活期存款',self.column[2],bank_data[bank_name]["DB_MY"],self.columns_en[2]])
				rows.append([total_data[0],bank_name,'國內外匯定期存款',self.column[3],bank_data[bank_name]["DB_FY"],self.columns_en[3]])
				#rows.append([total_data[0],bank_name,'國內外匯總存款',bank_data[bank_name]["DB_Y"]])
				rows.append([total_data[0],bank_name,'海外外匯活期存款',self.column[4],bank_data[bank_name]["OS_MY"],self.columns_en[4]])
				rows.append([total_data[0],bank_name,'海外外匯定期存款',self.column[5],bank_data[bank_name]["OS_FY"],self.columns_en[5]])
				#rows.append([total_data[0],bank_name,'海外外匯總存款',bank_data[bank_name]["OS_Y"]])

		self.output(rows)


	def parse2(self,source_path,destincation_path,date):
		self.set(source_path,destincation_path,date)
		book = xlrd.open_workbook(self.source_path+self.date+".xls")
...
if __name__ == '__main__':
	...
	for yy in range(95,100):
		for mm in range(1,13):
			date = '%d%02d'%(yy,mm)
			worker.parse(from_path,to_path,date)
	worker.parse(from_path,to_path,'10001')
	for yy in range(100,101):
		for mm in range(2,13):
			date = '%d%02d'%(yy,mm)
			worker.parse2(from_path,to_path,date)
