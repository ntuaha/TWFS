...
class TRANSFORM_LSME:
	source_type = 'LSME' 
	header=['年月','銀行','銀行類別','項目','數值',"欄位名稱"]
	rows = []
	total_data = [None]*15
	bank_data = {}
	modelist = ['本國銀行','外國銀行在台分行','大陸地區銀行在臺分行','信用合作社']
	columns = ["對中小企業","佔放款%","市佔率"]	
	columns_en = ["LN_SME","LN_SME_RATE","MARKET_RATE"]	
	...
	def parse(self):
		...
		for sheet_num in range(book.nsheets):
			...
			for i in range(sh.nrows):
				...
				self.total_data[1] = int(float(sh.cell_value(rowx=i,colx = 7))*1e6)
				self.total_data[2] = float(sh.cell_value(rowx=i,colx = 8))
				self.total_data[3] = float(sh.cell_value(rowx=i,colx = 9))
...
if __name__ == '__main__':
	#parser = TRANSFORM_LSME('/Users/aha/Dropbox/Project/Financial/Plan/rawdata/','/Users/aha/Dropbox/Project/Financial/Plan/data/')
	#parser.runParse('10001')
	#parser.runParse('10002')
	parser = TRANSFORM_LSME('/home/aha/Data/TWFS/rawdata/','/home/aha/Data/TWFS/data/')
	parser.parseAll()
