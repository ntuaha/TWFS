...
class TRANSFORM_CL_INFO:
	...
	columns = ["購置住宅貸款戶數","購置住宅貸款餘額","房屋修繕貸款戶數","房屋修繕貸款餘額","購置汽車貸款戶數"
	,"購置汽車貸款餘額","機關團體職工福利貸款戶數","機關團體職工福利貸款餘額","其他個人消費貸款戶數","其他個人消費貸款餘額"]	
	columns_en = ["Mortgage_Cnt","Mortgage_Bal","Decorator_Hse_Cnt","Decorator_Hse_Bal","Ln_Car_Cnt","Ln_Car_Bal","Ln_Worker_Cnt","Ln_Worker_Bal","Other_CL_Cnt","Other_CL_Bal"]
	...
	def parse(self):
		...
		for sheet_num in range(book.nsheets):
			...
			for i in range(sh.nrows):
				...
				self.total_data[7] = int(float(sh.cell_value(rowx=i,colx = 7)))
				self.total_data[8] = int(float(sh.cell_value(rowx=i,colx = 8))*1e6)
				self.total_data[9] = int(float(sh.cell_value(rowx=i,colx = 9)))
				self.total_data[10] = int(float(sh.cell_value(rowx=i,colx = 10))*1e6)
...
if __name__ == '__main__':
	#parser = TRANSFORM_CL_INFO('/Users/aha/Dropbox/Project/Financial/Plan/rawdata/','/Users/aha/Dropbox/Project/Financial/Plan/data/')
	#parser.runParse('10001')
	#parser.runParse('10002')
	parser = TRANSFORM_CL_INFO('/home/aha/Data/TWFS/rawdata/','/home/aha/Data/TWFS/data/')
	parser.parseAll()
