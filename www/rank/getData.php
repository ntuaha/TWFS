...
function GG($table,$column,$title){
	...
	$sql = sprintf("SELECT A.bank_code,B.bank_nm,%s,B.stock_type FROM %s as A left join bank_attr as B on (A.bank_code = B.bank_code) where A.data_dt='%s' and A.bank_nm not in ('總計','小計') order by %s desc,length(B.bank_nm) desc",$column,$table,$date2,$column);
	//echo "123<br/>";
	//echo $sql."<br>";
	...
	$rank = 1;
	$prev='';
	$sql = sprintf("SELECT A.bank_code,B.bank_nm,%s,B.stock_type FROM %s as A left join bank_attr as B on (A.bank_code = B.bank_code) where A.data_dt='%s' and A.bank_nm not in ('總計','小計') order by %s desc,length(B.bank_nm) desc",$column,$table,$date1,$column);
	$r = pg_query($dbconn,$sql);
	while($c = pg_fetch_array($r)){
		if($prev!=$c["bank_code"]){
