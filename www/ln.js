...
//AJAX START
      ...
      for (var i in ajaxdata){
        data.push([(parseFloat(ajaxdata[i].mortgage_bal))/1e6,parseFloat(ajaxdata[i].decorator_hse_bal)/1e6,parseFloat(ajaxdata[i].ln_car_bal)/1e6,parseFloat(ajaxdata[i].ln_worker_bal)/1e6,parseFloat(ajaxdata[i].other_cl_bal)/1e6]);
        names.push([i,"#EFEFEF"]);
      }
      //var data = getDataForMonth();
  ...
  // Set up variables required to draw a pie chart
    ...
    var arcs = d3CalendarGlobals.chartsGroup.selectAll("g.arc")
                  ...
                  .attr("transform", function (d) {
                      ...
                      return "translate(" +  (outerRadius + d3CalendarGlobals.gridXTranslation * 5 + cellPositions[currentDataIndex][0]) + ", " +  (outerRadius + d3CalendarGlobals.gridYTranslation * 1.75 + cellPositions[currentDataIndex][1]) + ")";
                  });
...
function renderCalendarGrid(month, year) {
 ...
 var d3CalendarGlobals = function() {
            ...
            function publicGridHeight() { return calendarHeight - 30; }
            function publicCellWidth() { return publicGridWidth() / 7; }
            function publicCellHeight() { return publicGridHeight() / 15; }
            ...
            function publicDaysInMonth() {
                ...
                var daysRequiredFromNextMonth = 100 - daysArray.length;
